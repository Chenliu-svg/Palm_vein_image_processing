...
y4=y1-(x1-x2);
%通过迭代扩大正方形ROI
original_x1=x1;original_y1=y1;%记录最初的(x1,y1)
while((J(y3,x3)==1)&&(J(y1,x1)==1))
    x1=x1+1;
    y1=original_y1-(x1-original_x1)*(y2-original_y1)/(original_x1-x2);
    y1=round(y1);%四舍五入
    x3=x2-(y2-y1);
    y3=y2-(x1-x2);
    x4=x1-(y2-y1);
...
imshow(I),title("在原图上绘制出最大化的正方形锚框");
hold on;
...
%显示截取得到的新图像
figure(7),imshow(new_img),title('自适应锚框截取的正方形ROI');
